...
正式分类只有：店内环境宣传、后湖湖边茶饮、后湖落日湖景、烧烤成品特写、烧烤烤制过程、烧烤食材展示、冰箱展示、晚霞、烧鸟烤串、卖花素材、生串、Live图、同行参考、其他待判断。
...
“同行参考”用于其他账号值得学习的图片/片段，必须保留原帖与出处，不作为自家发布素材。已有 hash 不重新分类；新分类仅作用于新增素材。


Live图是老板指定的成品类型分类。生成目录在顶层`素材库/Live图/`；审核时核对JPG/MOV配对与素材信息，不按菜品内容把两个配对文件分到不同类。按整组目录扫描及上传，保留原相对路径；MP4仅预览，验收截图不是独立Live。通用上传器只处理媒体，JSON来源记录当前留在本地，不能称已云同步。旧hash仍按远程正式分类，不覆盖。

Live配对入库命名按一组分配同一序号和基础名，JPG与MOV只扩展名不同；保持同目录，不能逐文件改成两个无关联名称。原文件保持只读。
